...
- [x] Persist the meal plan
  - [x] Persons are not stored
  - [x] Test that the meals in the plan have the same order like at generation.
- [x] What happens if I delete a person or recipe that is used in the meal plan? I need to handle this case. Maybe I can add a foreign key constraint and cascade delete the meals that use the deleted person or recipe. Or I can add a check before deleting a person or recipe and prevent the deletion if it is used in the meal plan.
- [x] Change activity level into table and remove the hardcoded mapping.
  - [x] Entities for the data
  - [x] Seed initial data for activity levels
  - [x] Map the entities to BL
    - [x] ImportError: cannot import name 'Person' from partially initialized module 'persons.person' (most likely due to a circular import) (C:\Dev\python_workshop_project\src\persons\person.py)
    - [x] I can not add a second person. It stops with the error "2026-04-01 15:47:43,761 | WARNING | A person with the name 'Test' already exists."
  - [x] Maybe optimize `ACTIVITY_LEVELS` into a better data structure
  - [x] Remove TODO in code
- [x] Remove YAML storage code
...
- [x] Are all TODOs in the code done? :x:
...
- [x] Check if the exception handling is well done
- [x] Are there license conflicts for new dependencies?
- [x] Is there any deactivated code?
- [x] Are all TODOs in the code done?
- [x] Are there enough meaningful comments
...
- [x] 1.1: :green_circle: Result: As expected
- [x] 1.2: :green_circle: Result: As expected
- [x] 1.3: :green_circle: Result: As expected
- [x] 2.1: :yellow_circle: Result: OpenFoodFacts-API does not work stable.
- [x] 2.2: :green_circle: Result: As expected
- [x] 2.3: :green_circle: Result: As expected

## :books: Documentation

- [x] Do I need a new PIA or update an existing one? :red_circle:
- [x] Update the README.md
  - [x] Document the usage of the model registry and how to add new entities.
...
- [x] Document the datamodel in an article
- [x] Document the unity of work pattern
- [x] Link the article in the README.md

### :bulb: Decisions

| Decision                                 | Cause                                                           |
| ---------------------------------------- | --------------------------------------------------------------- |
| All data is stored in a SQLite database. | A local database in one file is the optimal solution right now. |


| Decision                                            | Cause                                                     |
| --------------------------------------------------- | --------------------------------------------------------- |
| I will use the repository and unit of work pattern. | These patterns standardize the data and session handling. |
