...
- [x] Merge `feature` into `master` or `hotfix` into `production` and `master` and remove the `bugfix` branch
- [x] Check if the compiled artifact is valid
- [x] Cleanup the Git history locally on the dev system
