...
- [ ] Search for food
  ...
  - [ ] Now after every run the app crashes
  - [ ] Are queries allowed for every question or only for the first?
  - [ ] Memory of GPU is to low
    - [ ] Is the model loaded into memory every time an instance of the prompt is created?
  - [ ] Modell outputs it's thinking process
  - [ ] Make the prompt behavior configurable
  - [ ] Implement the selection part
  - [ ] Consider using a factory pattern here to standardize the creation of SQLiteRag instances.
- [ ] Create the embeddings in chunks
